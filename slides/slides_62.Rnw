...
Returning to our example where we flip a coin twice, let $X$ be the number of heads we observe. Our coin is \textit{not} fair, and the probability of getting a heads is 0.8.
