...
\begin{itemize}
\item Last class, we assumed we had a finite population that we observe all of, and the source of randomness in what we observed was due to random assignment of treatment. This is called \textit{randomization inference.}\pause
\item Now, we'll assume that our data is produced from a random generative process, where we're sampling from some (potentially infinite) population distribution that is not fully observed. This is the type of inference we use for survey sampling. \pause
...
\begin{itemize}
\item Returning to our example where we flip a coin twice, let $X$ be the number of heads we observe. Our coin is  \textit{not} fair, and the probability of getting a heads is 0.8. 
\pause
...
\item Here, where we have mutliple coin flips, we can compare that to the probability distribution of hires for two people with the same profile. 
\end{itemize}
