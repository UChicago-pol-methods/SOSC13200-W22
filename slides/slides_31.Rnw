...
\item Homework assignments
  \begin{itemize}
    ...
    \item Due dates changed to Monday (except for last assignment)
...
\begin{itemize}
\item Dean Knox \& Jonathan Mummolo wrote a letter critiquing the original article, based on the authors' failure to appropriately apply Bayes Rule;
