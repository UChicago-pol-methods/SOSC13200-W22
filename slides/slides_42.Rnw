...
\begin{itemize}
\item Duflo, Bannerjee, Kremer motivated by work of development agencies. \pause
...
\item Causation: $X$ \textit{moves} $Y$. E.g., when I fertilize the plants, I get a higher yield. 
\begin{itemize}
\item Implies a \textit{counterfactual}. \pause. If I \textit{hadn't} moved $X$, we would see a different outcome for $Y$. \pause
...
\item Independent variable: ($X$) in causal settings, the thing that we manipulate. Also ``treatment.'' In non-causal settings, something that may have some natural variation in the population associated with the dependent variable. \pause
\end{itemize}
...
\pause
Why is $\E[Y_i(0)]$ different from $\E[Y_i|X = 0]$? \pause
...
We're interested in the treatment effect, the difference in potential outcomes had we applied treatment $X$, vs. if we had not. \pause
...
\item What this give us: $\E[Y_i | X_i = 1] = \E[Y_i(1)]$ \pause
...
\item Why is treatment assigned at the school level? \pause
\end{itemize}
...
\begin{itemize}
\item In general, we would like to assume $Y_i(X_i)$ does not change depending on some $X_j$ for $j \neq i$. \pause
...
\item What is being measured? \pause
\end{itemize}
...
primary school pupils ($N_{dit}$) and the number of those pupils in schools ran-
domly assigned to deworming treatment ($N^T_{dit}$) within a certain distance from
